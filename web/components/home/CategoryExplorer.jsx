...
export function CategoryExplorer({ categories, onSelect }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: bp.ltDesktop ? "1fr" : "320px 1fr", background: "var(--white)", borderRadius: "var(--radius-xl)", overflow: "hidden", border: "1px solid var(--ink-100)", boxShadow: "var(--shadow-md)", minHeight: bp.ltDesktop ? 0 : 400 }}>
      <div
        style={{
          background: "var(--paper-alt)", padding: 12, display: "flex", flexDirection: bp.ltDesktop ? "row" : "column",
          gap: bp.ltDesktop ? 8 : 4, overflowX: bp.ltDesktop ? "auto" : "visible", overflowY: bp.ltDesktop ? "visible" : "auto",
          scrollbarWidth: "none", borderRight: bp.ltDesktop ? "none" : "1px solid var(--ink-100)", borderBottom: bp.ltDesktop ? "1px solid var(--ink-100)" : "none",
        }}
      >
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? "var(--white)" : "transparent", boxShadow: isActive ? "var(--shadow-sm)" : "none",
                cursor: "pointer", textAlign: "left", fontFamily: "var(--font-body)", flexShrink: bp.ltDesktop ? 0 : 1, whiteSpace: bp.ltDesktop ? "nowrap" : "normal",
              }}
            >
              ...
              <span style={{ fontSize: "var(--text-sm)", fontWeight: isActive ? "var(--weight-semibold)" : "var(--weight-medium)", color: isActive ? "var(--navy-900)" : "var(--ink-700)", flex: 1 }}>{cat.title}</span>
              {!bp.ltDesktop && <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--ink-300)" }}>{cat.count}</span>}
            </button>
          );
        })}
      </div>
      <div style={{ padding: bp.mobile ? "26px 22px" : "40px 44px", display: "flex", flexDirection: "column", gap: 16, position: "relative" }}>
