...
export function ITRPromo() {
  ...
  const bp = useBP();
  return (
    <section style={{ padding: bp.mobile ? "20px 20px 64px" : "40px 32px 90px" }}>
      <div style={{ maxWidth: "var(--container-max)", margin: "0 auto", position: "relative", borderRadius: "var(--radius-xl)", overflow: "hidden", minHeight: 340, display: "grid", gridTemplateColumns: bp.ltDesktop ? "1fr" : "1fr 1fr", background: "linear-gradient(115deg,var(--navy-050) 46%,#dbe6fb)" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, padding: bp.mobile ? "40px 24px" : "52px 20px 52px 56px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 18 }}>
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: "var(--weight-semibold)", fontSize: bp.mobile ? "var(--text-3xl)" : "var(--text-4xl)", color: "var(--navy-900)", margin: 0, lineHeight: 1.12, letterSpacing: "var(--tracking-tight)" }}>
            File ITR in Minutes
            <br />
        ...
        <div style={{ position: "relative", minHeight: bp.mobile ? 260 : 340, order: bp.ltDesktop ? -1 : 0, display: "block", background: "#f2e3ce" }}>
          <Image src="/images/itr-refund.jpg" alt="ITR refund" fill sizes="(max-width: 900px) 50vw, 600px" style={{ objectFit: "cover", objectPosition: bp.mobile ? "center 32%" : "center" }} />
        </div>
      </div>
    </section>
  );
