...
function ClientFace({ c, i, back }) {
  ...
  const bp = useBP();
  const av = bp.mobile ? 40 : 48;
  return (
    <div
      style={{
        position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: back ? "rotateY(180deg)" : "none",
        display: "flex", alignItems: "center", gap: bp.mobile ? 10 : 13, padding: bp.mobile ? "10px 12px" : "14px 16px", background: "var(--white)", border: "1px solid var(--ink-100)",
    ...
    >
      ...
      <div style={{ position: "relative", flexShrink: 0 }}>
        <div style={{ width: av, height: av, borderRadius: "50%", overflow: "hidden", border: "2px solid " + accent }}>
          <Portrait seed={c.av} size={av} />
        </div>
...
function ClientFlipCard({ card, i }) {
  ...
  const bp = useBP();
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ width: "100%", maxWidth: bp.mobile ? "none" : 250, height: 92, perspective: 1000, cursor: "default" }}>
      <div
        style={{
